...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    const { deviceId, lang = 'zh' } = body

    if (!deviceId) {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ success: false, error: '缺少设备ID' })
      }
    }

    // 创建 session
    const sessionId = Date.now() + Math.random()
    const shareCode = `CEO${sessionId.toString(36).toUpperCase()}`

    // 存储到云数据库
    await db.collection('sbti_sessions').add({
      data: {
        sessionId,
        deviceId,
        lang,
